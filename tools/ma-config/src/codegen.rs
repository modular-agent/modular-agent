...
/// Update the app's `Cargo.toml` with the selected modules as dependencies.
pub fn update_manifest(
    ...
    for module in &config.modules {
        let known = registry::find_by_name(&registry.modules, &module.name);
        deps.insert(
            &module.name,
            Item::Value(module_dep_value(app, module, known)),
        );
    }
...
/// The `[dependencies]` entry for one module.
///
/// Out-of-tree modules are always path dependencies on their clone under
/// `custom_modules/`; a path inside the workspace makes the clone a workspace
/// member, so it shares this workspace's lock file and its copy of core.
fn module_dep_value(app: AppKind, module: &ModuleEntry, known: Option<&KnownModule>) -> Value {
    let custom_features = module.crate_features.as_deref();
    ...
    let mut dep = source_to_inline_table(app, module);
    // The wizard stores `None` when the selection equals the registry's
    // default_features. Unlike in-tree modules, where the workspace dependency
...
/// Generate the app's `modules.rs` with one import per selected module.
pub fn generate_modules_rs(app: AppKind, config: &BuildConfig, root: &Path) -> Result<(), String> {
    let modules_path = app.modules_rs_path(root);

    let mut content = String::from(
        "//! Module crate imports (auto-generated by ma-config, do not edit)\n\
         //!\n\
         //! Each `use` pulls in the crate, causing #[modular_agent] registrations\n\
         //! to be linked via the `inventory` crate.\n\n",
    );
    for module in &config.modules {
        content.push_str(&format!("use {} as _;\n", module.rust_crate_name()));
    }

    fs::write(&modules_path, content)
        .map_err(|e| format!("Failed to write {}: {e}", modules_path.display()))
}

/// Ensure the CLI's `main.rs` declares the generated module module.
pub fn ensure_mod_modules(app: AppKind, root: &Path) -> Result<(), String> {
    let main_path = app.main_rs_path(root);
    let content = fs::read_to_string(&main_path)
        .map_err(|e| format!("Failed to read {}: {e}", main_path.display()))?;
    if content.contains("mod modules;") {
        return Ok(());
    }
    ...
    let after_use = lines
        ...
        .map(|i| i + 1)
        .unwrap_or(0);
    lines.insert(after_use, "mod modules;");
...
fn source_to_inline_table(app: AppKind, module: &ModuleEntry) -> InlineTable {
    let mut dep = InlineTable::new();
    match &module.source {
        Some(ModuleSource::Workspace) => {
            dep.insert("workspace", Value::from(true));
        }
        ...
        Some(ModuleSource::Path { path }) => {
            dep.insert("path", Value::from(rebase(app, path)));
        }
        // A registry module carries no source: it is the clone under
        // custom_modules/, which validate_paths has already checked for.
        None => {
            dep.insert(
                "path",
                Value::from(rebase(app, &registry::clone_path(&module.name))),
            );
        }
    }
    dep
...
/// Check that every out-of-tree module points at a usable crate.
///
...
pub fn validate_paths(config: &BuildConfig, root: &Path) -> Result<(), String> {
    ...
    for module in &config.modules {
        let path = match &module.source {
            Some(ModuleSource::Workspace) => continue,
            Some(ModuleSource::Path { path }) => path.clone(),
            None => registry::clone_path(&module.name),
        };

        let manifest = root.join(&path).join("Cargo.toml");
        let Ok(manifest) = fs::read_to_string(&manifest) else {
            problems.push(format!(
                "{}: no crate at '{path}' — clone it (see custom_modules/README.md) or drop it \
                 from the selection",
                module.name
            ));
            continue;
        ...
        if !core_is_local(&manifest) {
            problems.push(format!(
                "{}: '{path}' still takes modular-agent-core from crates.io. Point it at the \
                 ...
                 modules registered in the other one silently disappear.",
                module.name
            ));
        }
    }
...
mod tests {
        ...
        Registry {
            modules: vec![
                KnownModule {
                    ...
                    description: "Standard modules".into(),
                    in_tree: true,
                ...
                KnownModule {
                    ...
                    description: "Lifelog modules".into(),
                    in_tree: false,
                ...
                KnownModule {
                    ...
                    description: "Example modules".into(),
                    in_tree: false,
    ...
    #[test]
    fn in_tree_modules_become_workspace_dependencies() {
        ...
        // Modules dropped from the selection lose their dependency line.
    ...
    #[test]
    fn unregistered_module_paths_are_rebased_to_the_app_manifest() {
        let config = BuildConfig {
            modules: vec![entry(
                "modular-agent-custom",
                Some(ModuleSource::Path {
                    // Backslashes from a Windows path are normalized for Cargo.
                    path: "custom_modules\\modular-agent-custom".into(),
                }),
                None,
            )],
        };
        ...
        assert!(out.contains(
            "modular-agent-custom = { path = \"../../../custom_modules/modular-agent-custom\" }"
        ));
    }

    #[test]
    fn registry_modules_resolve_to_their_custom_modules_clone() {
        let config = BuildConfig {
            modules: vec![entry("modular-agent-lifelog", None, None)],
        };
        let out = manifest_after("clone-path", AppKind::Desktop, &config);

        assert!(out.contains(
            "modular-agent-lifelog = { path = \"../../../custom_modules/modular-agent-lifelog\" }"
        ));
    ...
    fn registry_default_features_materialize_for_out_of_tree_deps() {
        let config = BuildConfig {
            modules: vec![entry("modular-agent-example", None, None)],
        };
        let out = manifest_after("registry-defaults", AppKind::Cli, &config);

        assert!(out.contains(
            "modular-agent-example = { path = \"../../custom_modules/modular-agent-example\", \
    ...
    fn validate_with_clone(label: &str, core_dep: &str) -> Result<(), String> {
        let dir = scratch(label);
        let crate_dir = dir.join("custom_modules/modular-agent-lifelog");
    ...
    /// deletes its entry.
    #[test]
    fn a_missing_clone_points_at_the_custom_modules_readme() {
        ...
        assert!(err.contains("no crate at 'custom_modules/modular-agent-lifelog'"));
        assert!(err.contains("custom_modules/README.md"));
    }
