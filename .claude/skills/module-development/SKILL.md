...
description: Modular Agent development reference — the #[modular_agent] macro, AsModule trait and lifecycle, doc-comment rules for module descriptions, UI hints, config types and access, Error handling, and the DB connection caching pattern. Use when writing, modifying, or reviewing modules in this workspace.
---

# Module Development

All modules use the `#[modular_agent]` macro from `modular-agent-core`:
...
impl AsModule for MyModule {
    ...
    async fn process(&mut self, ctx: ModuleContext, port: String, value: Value) -> Result<(), Error> {
...
- `///` doc comments on the struct automatically become `ModuleDefinition::description`; no need to use `description = "..."` in the macro attribute.
- Explicit `description = "..."` overrides the doc comment when both are present.
- Struct doc comments are **user-facing documentation**, rendered as-is in the UI (markdown-rendered in the desktop inspector and Settings → Module screens). Write them for the person wiring up the workflow, not for the person maintaining the code.

All modules should have doc comments following this structure:
...
- `# Ports`: Required when the module has input/output ports
- `# Configuration`: Required when the module has configs
- `# Global Configuration`: Required when the module uses `custom_global_config` / `string_global_config`
...
**Reference implementations:** `ResponsesModule` (`crates/modular-agent-llm/src/responses.rs`), `SlackPostModule` (`modular-agent-slack/src/modules.rs`)
...
- Translations will ship as package-bundled `locales/<lang>.json` catalogs (module name → title / description / config descriptions), resolved by core as an overlay in `get_module_definitions`. The `ModuleDefinition` data model is unchanged; untranslated keys fall back to English.
- A catalog format (same shape as Node-RED / VS Code extensions) rather than per-module markdown files, because the translation targets include `title` and config descriptions, not just `description`.
...
Definition-level `hints` apply to all instances of the module type.
Instance-level overrides use `ModuleSpec.extensions`.
...
impl AsModule for MyModule {
    ...
    // Optional: Called when module starts
    ...
    fn configs_changed(&mut self) -> Result<(), Error> { Ok(()) }
...
```rust
Error::InvalidValue("Invalid input format".into())
Error::InvalidConfig("Missing required config".into())
Error::IoError(format!("Network error: {}", e))
```

## Connection Caching Pattern (DB Modules)

All database modules use this pattern for connection pooling:
...
async fn get_client(url: &str) -> Result<Client, Error> {
