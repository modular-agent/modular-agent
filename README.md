...
Build AI workflows like a modular synth — patch extensible modules together visually into real-time pipelines. LLMs, databases, web scrapers, messaging, and more. Privacy-first, no cloud required.
...
Modular Agent is a desktop app and Rust framework for composing modules into an agent the way you wire modules on a synthesizer. Drop modules onto a canvas, connect their ports, and flip the Run switch — values stream through the graph in real time, and a patch keeps running like a live pipeline rather than a one-shot script. Everything executes on your machine; LLM endpoints, databases, and messaging services are reached only when you patch them in.

## How It Works

- **Patch** — a workflow saved as JSON: a set of modules plus the connections between them.
- **Module** — a processing unit instantiated from a module definition, with named input/output ports and a config.
- **Connection** — wires an output port to an input port. Targeting the special `config:<key>` handle streams values into a module's config instead.
- Values (`Value`) stream through the graph asynchronously; each external trigger carries a `ModuleContext` that identifies its flow across modules end to end.
...
[recommended module packages](custom_modules/README.md#recommended-module-repositories).
...
For manual build steps and changing the module selection, see the
...
- **[Documentation site](https://modular-agent.github.io/docs/)** — [Installation](https://modular-agent.github.io/docs/getting-started/installation/), [Your First Patch](https://modular-agent.github.io/docs/getting-started/first-patch/), [Using the Chat Module](https://modular-agent.github.io/docs/getting-started/chat-patch/)
...
- [Module libraries](#module-libraries) — everything that can be patched in; [custom_modules/README.md](custom_modules/README.md) explains how out-of-tree packages are built
...
| [`crates/modular-agent-core`](crates/modular-agent-core) | `modular-agent-core` | Orchestration engine, module runtime, patch loader |
| [`crates/modular-agent-macros`](crates/modular-agent-macros) | `modular-agent-macros` | `#[modular_agent]` procedural macro |
| [`crates/modular-agent-std`](crates/modular-agent-std) | `modular-agent-std` | Standard utility modules |
| [`crates/modular-agent-llm`](crates/modular-agent-llm) | `modular-agent-llm` | OpenAI / Claude / Ollama modules |
...
| [`tools/ma-config`](tools/ma-config) | `ma-config` | Module selection / build configuration TUI |

## Module Libraries

Modules come in packages. `std` and `llm` live in this repository and are part of every build. The rest live in their own repositories under [github.com/modular-agent](https://github.com/modular-agent): clone the ones you want into `custom_modules/` and select them with the ma-config wizard — see [custom_modules/README.md](custom_modules/README.md) for details.

| Category | Package | Modules |
|---|---|---|
...
| General | [modular-agent-web](https://github.com/modular-agent/modular-agent-web) | Web/HTTP, scraping, search, YouTube modules |
| General | [modular-agent-monty](https://github.com/modular-agent/modular-agent-monty) | Monty script modules |
| General | [modular-agent-zapcode](https://github.com/modular-agent/modular-agent-zapcode) | ZapCode TypeScript script modules |
| Messaging | [modular-agent-slack](https://github.com/modular-agent/modular-agent-slack) | Slack messaging modules |
| Messaging | [modular-agent-mattermost](https://github.com/modular-agent/modular-agent-mattermost) | Mattermost messaging modules |
| Data / Media | [modular-agent-lifelog](https://github.com/modular-agent/modular-agent-lifelog) | Screen capture, window tracking modules |
| Databases | [modular-agent-sqlx](https://github.com/modular-agent/modular-agent-sqlx) | SQL database modules (PostgreSQL, MySQL, SQLite) |

Nothing is limited to this organization — any repository holding a module crate can be cloned into `custom_modules/` and picked up by the wizard.
