...
Out-of-tree module crates live here as plain git clones. Everything in this
...
A clone that no app selects is inert; cargo never looks at it. Each module repo
depends on `modular-agent-core` by path, so there is exactly one linked copy of
core and one `inventory` module registry.

## Recommended module repositories
...
| Repository | Modules |
| --- | --- |
| `modular-agent-lifelog` | Screen capture, window tracking modules |
| `modular-agent-mattermost` | Mattermost messaging modules |
| `modular-agent-monty` | Monty script modules |
| `modular-agent-slack` | Slack messaging modules |
| `modular-agent-sqlx` | SQL database modules (PostgreSQL, MySQL, SQLite) |
| `modular-agent-web` | Web/HTTP, scraping, search, YouTube modules |
| `modular-agent-zapcode` | ZapCode TypeScript script modules |

Nothing here is limited to that organization: any repository holding a module
crate can be cloned into this directory, and the wizard offers every subdirectory
that has a `Cargo.toml`.

## ma-registry.yaml

A module repository describes itself to the wizard with an `ma-registry.yaml`
at its root — the catalog entry lives with the crate, not in this repository:

```yaml
name: modular-agent-lifelog
description: Screen capture, window tracking modules
available_features:
  - application
  - screen
default_features:
