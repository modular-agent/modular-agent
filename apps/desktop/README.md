...
- 🎨 **Visual Workflow Editor** — Node-based drag-and-drop interface for designing module pipelines
...
- ⌨️ **Customizable Shortcuts** — All hotkeys rebindable in Settings, including Quick Add slots for placing frequent modules
...
The default build links the in-tree module crates ([modular-agent-std](../../crates/modular-agent-std), [modular-agent-llm](../../crates/modular-agent-llm)). More module packages — web scraping, messaging, databases, screen capture, script modules — are added at build time:

1. Clone the module repositories you want into `custom_modules/` at the repository root. See [custom_modules/README.md](../../custom_modules/README.md) for the repository list.
2. Run the **ma-config** TUI wizard to select modules and per-crate features (only cloned modules are offered):
...
- The wizard updates `src-tauri/Cargo.toml` and regenerates `src-tauri/src/modules.rs` (never edit that file by hand). In-tree modules are emitted as `{ workspace = true }`; out-of-tree modules as `path = "../../../custom_modules/<name>"`, which makes each clone a member of the workspace.
...
- **Core** — [modular-agent-core](../../crates/modular-agent-core) module runtime, accessed through [tauri-plugin-modular-agent](../../crates/tauri-plugin-modular-agent)
...
src/                          # Svelte frontend
  ...
  lib/
    ...
    shared.svelte.ts          # global module event bus
    ...
    components/
      patch-editor/           # editor internals
        ...
        module-node.svelte     # module node rendering
      module-list/             # module category tree popup
      ui/                     # shadcn-svelte components (generated)
src-tauri/src/                # Rust backend
  modular_agent_desktop/
    app.rs                    # patch management state (ModularAgentApp)
    settings.rs               # core settings, module global configs
...
The frontend calls Rust commands through Tauri's `invoke()` (wrapped in `src/lib/modular_agent.ts`), which reach the engine via `tauri-plugin-modular-agent`. On the way back, `observer.rs` subscribes to engine `ModularAgentEvent`s and relays them as Tauri events: `ma:module_config_updated`, `ma:module_error`, `ma:module_in`, `ma:module_spec_updated` for per-module activity, and `ma:patch_list_changed`, `ma:patch_structure_changed`, `ma:patch_removed`, `ma:patch_renamed`, `ma:patch_running_changed` for patch lifecycle. Every payload carries an `origin` (`"desktop"`, `"mcp"`, …) so the frontend can tell its own echoes from external edits.
...
- `shared.svelte.ts` is the global event bus: Tauri listeners update per-module state regardless of which tab is visible.
- External edits (an MCP module, another window) merge into an open canvas as a diff: origin-based self-echo filtering plus `reconcileFlow`, preserving undo history and selection.
...
- **Keyboard shortcuts** — every editor hotkey is rebindable; Quick Add slots (`mod+1`–`mod+5`) bind both a key and a module type.
...
The core engine ships a built-in MCP server (Settings → Core → MCP Server) so external AI agents can inspect module definitions, build and edit patches, and verify running flows. Enabling it auto-generates a Bearer token; connect from Claude Code with:
...
Module packages can replace the default node rendering with their own Svelte 5 components via [`@modular-agent/widget-kit`](widget-kit/README.md):

- **NodeView** — a custom body for a specific module type
...
- [modular-agent-core](../../crates/modular-agent-core) — orchestration engine and module runtime
...
- [Module libraries](../../README.md#module-libraries) — the full package list
