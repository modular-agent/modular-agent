...
<script lang="ts">
  ...
  import { getModuleSpec } from "tauri-plugin-modular-agent-api";
  import type { ModuleSpec } from "tauri-plugin-modular-agent-api";
  ...
  import ModuleConfig from "./module-config.svelte";
  ...
  const moduleDef = $derived(moduleDefs[data?.def_name] ?? null);
  // const description = $derived(moduleDef.description);

  let errorMessages = $state<string[]>([]);
  let inputMessage = $state<string>("");
  let inputCount = $state(0);

  const moduleEvent = sharedModuleEvents.getModule(nodeId);
  ...
  $effect.pre(() => {
    const { key, value, seq } = moduleEvent.configUpdated;
    if (!seq) return;
    untrack(() => {
      let currentValue = data.configs?.[key];
      if (currentValue === value) return;
      // Snapshot: `value` read from the deeply-reactive modules store is a $state
  ...
  $effect(() => {
    const { port, seq } = moduleEvent.input;
    if (!seq) return;
    untrack(() => {
      inputMessage = port;
      inputCount += 1;
  ...
  // Spec updated — refetch module spec from backend
  $effect(() => {
    const seq = moduleEvent.specUpdated;
    if (!seq) return;
    getModuleSpec(id).then((spec) => {
      if (spec) {
  ...
  onDestroy(() => sharedModuleEvents.removeModule(nodeId));
  ...
  // Custom NodeView registered for this module type (replaces the default
  // config iteration in the contents area when present).
  const NodeView = $derived(getNodeView(data.def_name));

  let hide_title = $derived(moduleDef?.hide_title ?? false);
          ...
          <div class="flex flex-row space-x-2">
            {#if editTitle}
              <Input
                ...
                type="text"
                value={data.title ?? moduleDef.title ?? data.def_name}
                autofocus
                ...
                onkeydown={(evt) => {
                  if (evt.key === "Enter") {
                    ...
                    if (newTitle === "" || newTitle === (moduleDef.title ?? data.def_name)) {
            ...
            {:else}
              ...
              >
                <div class="text-xl font-semibold" style={titleColorStyle}>
                  {data.title ?? moduleDef.title ?? data.def_name}
                </div>
              </button>
  ...
  {:else if data.configs}
    <form class="grow flex flex-col gap-1 pl-7 pr-7 pb-4">
      {#each Object.entries(data.configs) as [key, value]}
        <ModuleConfig name={key} {value} configSpec={data.config_specs?.[key]} {updateConfig} />
      {/each}
    </form>
  {/if}
{/snippet}
