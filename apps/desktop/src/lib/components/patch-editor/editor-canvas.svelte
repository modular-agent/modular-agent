<script lang="ts">
  ...
  import { getPatchSpec } from "tauri-plugin-modular-agent-api";
  ...
  const actions: ActionEntry[] = [
    {
      id: "editor.save",
      handler: () => editor.savePatch(),
      preventDefault: true,
    },
    {
      id: "editor.toggle_run",
      handler: () => {
        if (editor.running) editor.stopPatch();
        else editor.startPatch();
      },
      preventDefault: true,
    },
    {
  ...
  const handleOnDelete: OnDelete<PatchNode, PatchEdge> = async ({ nodes, edges }) => {
    await editor.handleOnDelete({ nodes, edges });
  };

  function handleBeforeConnect(connection: Connection): PatchEdge {
  ...
  const handleNodeContextMenu: NodeEventWithPointer<MouseEvent, PatchNode> = ({ event, node }) => {
    event.preventDefault();
  ...
  function handleSelectionDragStart(_event: MouseEvent | TouchEvent, nodes: PatchNode[]) {
    editor.handleNodeDragStart(nodes);
  }
  ...
  const handleSelectionDragStop: OnSelectionDrag = async (_event, draggedNodes) => {
    await editor.handleSelectionDragStop(draggedNodes as PatchNode[]);
    updateMiniMapVisibility();
  };
  ...
  async function handleSaveAsSubmit() {
    ...
    const s = await getPatchSpec(editor.patch_id);
    if (!s) return;
    const new_id = await saveAsPatch(editor.saveAsName, s);
    if (!new_id) return;
...
>
  ...
  >
    ...
    <PaneContextMenu
      ...
      {hotkeys}
      onstart={() => editor.startPatch()}
      onstop={() => editor.stopPatch()}
      onnew={() => editor.showNewPatchDialog()}
      onsave={() => editor.savePatch()}
      onsaveas={() => editor.showSaveAsDialog()}
      onimport={() => editor.importPatchAndNavigate()}
      onexport={() => editor.exportPatch()}
...
<PatchActionDialog
  action="New"
  name={editor.name}
  onAction={(name) => editor.newPatchAndNavigate(name)}
  bind:open={editor.openNewPatchDialog}
/>
