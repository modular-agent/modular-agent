...
export class DeleteCommand implements Command {
  label: string;
  private deletedNodes: PatchNode[];
  private deletedEdges: PatchEdge[];

  constructor(
    private patchId: string,
    deletedNodes: PatchNode[],
    deletedEdges: PatchEdge[],
    label?: string,
  ) {
...
export class CutCommand extends DeleteCommand {
  constructor(patchId: string, deletedNodes: PatchNode[], deletedEdges: PatchEdge[]) {
    super(patchId, deletedNodes, deletedEdges, "Cut");
  }
}
...
export class AddConnectionCommand implements Command {
  ...
  async execute(editor: EditorState) {
    ...
    await addConnection(this.patchId, edgeToConnectionSpec(newEdge));
    editor.edges = [...editor.edges, newEdge];
    this.edge = { ...newEdge };
  }

  async undo(editor: EditorState) {
    await removeConnection(this.patchId, edgeToConnectionSpec(this.edge));
...
export class PasteCommand implements Command {
  ...
  private pastedNodes: PatchNode[];
  private pastedEdges: PatchEdge[];
...
// --- History cache (per patch ID) ---

const historyCache = new Map<string, CommandHistory>();

export function getOrCreateHistory(patchId: string, maxLength: number): CommandHistory {
  let h = historyCache.get(patchId);
  if (!h) {
    h = new CommandHistory(maxLength);
    historyCache.set(patchId, h);
  }
  return h;
}

export function removeHistory(patchId: string): void {
  historyCache.delete(patchId);
}
