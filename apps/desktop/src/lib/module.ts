...
import {
  ...
  type PatchSpec,
  type ConnectionSpec,
  getModuleDefinitions as getModuleDefinitionsAPI,
...
export function patchToFlow(info: PatchInfo, spec: PatchSpec): PatchFlow {
  ...
  const nodes = spec.modules.map((module) => moduleSpecToNode(module));
...
export function defaultNodeSize(
  ...
  const hints = _moduleDefinitions?.[defName]?.hints;
...
export function moduleSpecToNode(spec: ModuleSpec): PatchNode {
  ...
  const background = _moduleDefinitions?.[spec.def_name]?.hints?.background === true;
  return {
...
export const NODE_COLOR_VALUES: Record<number, string> = {
  1: "var(--color-module-1)",
  2: "var(--color-module-2)",
  3: "var(--color-module-3)",
  4: "var(--color-module-4)",
  5: "var(--color-module-5)",
  6: "var(--color-module-6)",
  7: "var(--color-module-7)",
};

// Kind-based default color: module kind → CSS color variable
export const KIND_COLOR_VALUES: Record<string, string> = {
  default: "var(--color-module-4)",
  External: "var(--color-module-5)",
  Local: "var(--color-module-5)",
  Display: "var(--color-module-3)",
  Input: "var(--color-module-6)",
  UI: "var(--color-module-2)",
};

// Kind-based default color: module kind → palette index (numeric)
...
export function resolveNodeColor(data: ModuleSpec, moduleDef: ModuleDefinition | null): string {
  const c = resolveColorCss(data.color);
  if (c) return c;
  const h = moduleDef?.hints?.color;
  if (typeof h === "number" && NODE_COLOR_VALUES[h]) return NODE_COLOR_VALUES[h];
  return KIND_COLOR_VALUES[moduleDef?.kind ?? "default"] ?? KIND_COLOR_VALUES.default;
}
...
export function resolveNodeBgColor(
  ...
  return resolveColorCss(data.bg_color) ?? resolveColorCss(moduleDef?.hints?.bg_color) ?? null;
}
...
export function resolveNodeFgColor(
  ...
  return resolveColorCss(data.fg_color) ?? resolveColorCss(moduleDef?.hints?.fg_color) ?? null;
}
...
const EDGE_COLOR_MAP: Record<string, string> = {
  ...
  // unit
  unit: "var(--color-module-2)",
  // boolean
  boolean: "var(--color-module-3)",
  // number
  integer: "var(--color-module-6)",
  number: "var(--color-module-6)",
  // string-like
  string: "var(--color-module-5)",
  text: "var(--color-module-5)",
  // object-like
  object: "var(--color-module-4)",
  doc: "var(--color-module-4)",
  message: "var(--color-module-4)",
  // image
  image: "var(--color-module-4)",
  // error
  err: "var(--color-module-1)",
};
...
export function inferTypeForDisplay(spec: ModuleConfigSpec | undefined, value: any): string {
  let ty = spec?.type;
  ...
  if (value === undefined) {
    ...
    for (const v of value) {
      tys.add(inferTypeForDisplay({} as ModuleConfigSpec, v));
    }
    if (tys.size === 1) {
...
let _moduleDefinitions: ModuleDefinitions | null = null;
let _globalConfigsMap: ModuleConfigsMap | null = null;
...
export function getModuleDefinitions(): ModuleDefinitions {
  if (!_moduleDefinitions) {
    throw new Error("getModuleDefinitions() called before initGlobals()");
  }
  return _moduleDefinitions;
}

export function getGlobalConfigsMap(): ModuleConfigsMap {
...
export async function initGlobals() {
  ...
  _moduleDefinitions = await getModuleDefinitionsAPI();
