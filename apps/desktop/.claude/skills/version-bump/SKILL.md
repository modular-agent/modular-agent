...
description: >-
  ...
  apps/desktop/package.json, package-lock.json, src-tauri/Cargo.toml, src-tauri/tauri.conf.json
  and the workspace Cargo.lock. Use this whenever the user wants to change, raise, or set
...
Paths are relative to the workspace root (the directory holding the `[workspace]`
`Cargo.toml`); the lockfile is the workspace's, shared with every other crate.

| File | Field(s) |
|------|----------|
| `apps/desktop/package.json` | top-level `"version"` |
| `apps/desktop/package-lock.json` | **two** fields: top-level `"version"` and `packages[""].version` |
| `apps/desktop/src-tauri/tauri.conf.json` | top-level `"version"` |
| `apps/desktop/src-tauri/Cargo.toml` | `[package]` → `version = "..."` |
| `Cargo.lock` | the `[[package]]` block whose `name = "modular-agent-desktop"` → its `version = "..."` |
...
- `patch` / `minor` / `major` → read the current version from `apps/desktop/package.json`
  and step it (0.16.0 + minor → 0.17.0, + patch → 0.16.1, + major → 1.0.0).
- State the resolved target back to the user in your summary so a misread is obvious.

The current version is whatever is in `apps/desktop/package.json`.
...
```bash
grep -n '"version"' apps/desktop/package.json                      # -> NEW
grep -n '"version"' apps/desktop/package-lock.json | head -2       # top-level + root pkg -> NEW
grep -n '"version"' apps/desktop/src-tauri/tauri.conf.json         # -> NEW
grep -n '^version' apps/desktop/src-tauri/Cargo.toml               # [package] -> NEW
grep -n -A1 'name = "modular-agent-desktop"' Cargo.lock            # entry -> NEW
```
...
```bash
grep -n -A1 'name = "modular-agent-desktop"' Cargo.lock            # not OLD
```

### 4. Show the diff and hand back

```bash
git diff --stat -- apps/desktop/package.json apps/desktop/package-lock.json \
  apps/desktop/src-tauri/Cargo.toml apps/desktop/src-tauri/tauri.conf.json Cargo.lock
```

Show it to the user. **Do not commit or push** — that is the user's call. Tags in this
repository are per-component, so a desktop release is tagged `desktop-v<NEW>`:

```bash
git commit -am "chore(release): desktop <NEW>"
