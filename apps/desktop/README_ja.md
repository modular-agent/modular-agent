...
- 🎨 **ビジュアルワークフローエディタ** — ノードベースのドラッグ＆ドロップでモジュールパイプラインを設計
...
- ⌨️ **カスタマイズ可能なショートカット** — すべてのホットキーを Settings で変更可能。よく使うモジュールを配置する Quick Add スロットも
...
標準ビルドには in-tree のモジュール crate（[modular-agent-std](../../crates/modular-agent-std)、[modular-agent-llm](../../crates/modular-agent-llm)）が組み込まれています。さらに多くのモジュールパッケージ — Web スクレイピング、メッセージング、データベース、スクリーンキャプチャ、スクリプトモジュール — はビルド時に追加できます:

1. 使いたいモジュールリポジトリをリポジトリルートの `custom_modules/` に clone します。リポジトリ一覧は [custom_modules/README.md](../../custom_modules/README.md) を参照してください。
2. **ma-config** TUI ウィザードを実行し、モジュールと crate ごとの feature を選択します（clone 済みのモジュールだけが表示されます）:
...
- ウィザードは `src-tauri/Cargo.toml` を更新し、`src-tauri/src/modules.rs` を再生成します（このファイルを手で編集しないこと）。in-tree モジュールは `{ workspace = true }`、out-of-tree モジュールは `path = "../../../custom_modules/<name>"` として出力され、各 clone が workspace のメンバーになります。
...
- **コア** — [modular-agent-core](../../crates/modular-agent-core) モジュールランタイム。[tauri-plugin-modular-agent](../../crates/tauri-plugin-modular-agent) 経由でアクセス
...
src/                          # Svelte フロントエンド
  ...
  lib/
    ...
    shared.svelte.ts          # グローバルなモジュールイベントバス
...
フロントエンドは Tauri の `invoke()`（`src/lib/modular_agent.ts` でラップ）で Rust コマンドを呼び、`tauri-plugin-modular-agent` を通じてエンジンに届きます。戻りの経路では `observer.rs` がエンジンの `ModularAgentEvent` を購読し、Tauri イベントとして中継します: モジュール単位のアクティビティが `ma:module_config_updated`、`ma:module_error`、`ma:module_in`、`ma:module_spec_updated`、パッチのライフサイクルが `ma:patch_list_changed`、`ma:patch_structure_changed`、`ma:patch_removed`、`ma:patch_renamed`、`ma:patch_running_changed`。すべてのペイロードは `origin`（`"desktop"`、`"mcp"` など）を持ち、フロントエンドは自身のエコーと外部からの編集を区別できます。
...
- `shared.svelte.ts` はグローバルイベントバスで、どのタブが表示中かに関わらず Tauri リスナーがモジュール単位の状態を更新します。
...
- **キーボードショートカット** — エディタの全ホットキーを変更可能。Quick Add スロット（`mod+1`〜`mod+5`）はキーとモジュール種別の両方を割り当てられます。
- **自動起動** — アプリ起動時に実行するパッチを指定できます。
- **システムトレイ** — ウィンドウを閉じても、アプリと実行中のパッチはトレイで生き続けます。

## MCP サーバー

コアエンジンは内蔵 MCP サーバーを備えています（Settings → Core → MCP Server）。外部 AI エージェントはモジュール定義の参照、パッチの構築・編集、実行中フローの動作確認を行えます。有効化すると Bearer トークンが自動生成されます。Claude Code からの接続:
...
モジュールパッケージは [`@modular-agent/widget-kit`](widget-kit/README.md) を使って、デフォルトのノード描画を自前の Svelte 5 コンポーネントで置き換えられます:

- **NodeView** — 特定のモジュール種別のカスタムボディ
- **ConfigWidget** — config の値型ごとのカスタム入力
- **NodeStyle** — ノードフレームのプレゼンテーション上書き

UI パッケージはモジュールリポジトリ内の `ui/` npm パッケージとして配布され、ma-config の選択に基づいてビルド時に取り込まれます — 動的ロードやレジストリアクセスはありません。UI パッケージ固有の npm 依存は、初回のビルドまたは dev 起動時に `ui/node_modules` へ自動インストールされます。

## 関連

- [modular-agent-core](../../crates/modular-agent-core) — オーケストレーションエンジンとモジュールランタイム
- [tauri-plugin-modular-agent](../../crates/tauri-plugin-modular-agent) — このアプリの土台となる Tauri ブリッジ
- [`ma` CLI](../cli) — 同じパッチをヘッドレスで実行
- [モジュールライブラリ](../../README_ja.md#モジュールライブラリ) — パッケージの全一覧
