...
Modular Agent Desktop is a Tauri 2 desktop application that composes modules into an agent. It provides a visual editor for creating and managing patches.
...
# Custom build (select module packages to include)
...
- **Core**: `modular-agent-core` crate provides the module runtime
...
src/                    # Svelte frontend
  routes/               # SvelteKit pages
    patch_editor/      # Visual module workflow editor (thin page shell)
    open_patches/       # Patch file browser
    settings/           # App settings
    logs/               # Log viewer (production debug)
  lib/
    module.ts            # Tauri command wrappers, patch conversion utilities, setCoreSettings (partial merge + store update)
    ...
    shared.svelte.ts    # Global module event bus ($state + seq counter, replaces old writable stores)
    tab-store.svelte.ts # Multi-tab state singleton (tabs, activeTabId)
    modular_agent.ts    # Low-level Tauri invoke wrappers
    types.ts            # TypeScript type definitions
    components/
      patch-editor/    # Patch editor components (context-based, multi-instance)
        ...
        editor-canvas.svelte  # SvelteFlow canvas, keyboard shortcuts, module list popup
        editor-header.svelte  # Header bar (menubar, patch name, play/stop, status)
        module-node.svelte     # Module node component with event subscriptions
        module-config.svelte   # Module config input/display (node view, with Handles)
        sidebar-config.svelte # Module config input (sidebar, input renderers only, no readonly/hidden)
      ...
      module-list/       # Module category tree (popup, click to add)
...
src-tauri/src/          # Rust backend
  ...
  modular_agent_desktop/
    app.rs              # ModularAgentApp state, patch management
    settings.rs         # CoreSettings, module global configs
...
The patch editor keeps per-tab instances alive using CSS visibility toggling. Tab switching is instant — no component destruction/recreation.

- **`EditorHost`** (`editor-host.svelte`): Renders all open tabs simultaneously via `{#each tabStore.tabs}`. Active tab gets `visibility: visible`, others get `visibility: hidden` (not `display: none` — SvelteFlow needs layout dimensions). Loads patch data from backend on tab open, cleans up on tab close. Uses `untrack()` to avoid `$effect` infinite loops when managing the `flows` record.
...
- **`shared.svelte.ts`**: Global module event bus using `$state` + monotonic `seq` counter. `SharedModuleEvents` class keyed by module ID. Tauri listeners update entries regardless of tab visibility — non-active tabs continue receiving backend events (nodes stay mounted).
- **`tab-store.svelte.ts`**: Global tab state singleton. Navigation entry points (`nav-patches`, `editor-canvas`, `context.svelte.ts`) call `tabStore.openTab()` before `goto()`. The `[id]/+page.svelte` route only handles deep-link tab activation — its `$effect` uses `untrack()` to avoid reacting to `tabStore.tabs` changes (only reacts to route param changes). Without this, `closeTab` modifying `tabs` would re-trigger the deep-link handler before navigation completes.
- **Tab close lifecycle**: `closeTab()` removes tab from `tabStore.tabs` → `editor-host` `$effect` removes `flows` entry and calls `closePatch(id)` (fire-and-forget, only for stopped patches) → backend `close_patch` calls `remove_patch` (idempotent) and removes name→ID mapping. If `loadFlow` was still in progress when tab closed, the `finally` block handles backend cleanup.
...
- **Config components**: `module-config.svelte` (node view, has display + input renderers and Handles) and `sidebar-config.svelte` (sidebar, input renderers only — skips readonly/hidden configs). Adding a new input renderer type requires updating both.
...
- **Initial node size**: `AddModuleCommand.execute()` uses `ModuleDefinition.hints.width/height` (grid unit multipliers) when available, falling back to 1 grid unit (`snapGridSize`). Pre-existing `spec.width/height` from `newModuleSpec()` takes priority over hints. With `hints.free_size`, `width`/`height` are pixel values instead of grid multipliers (fallback 120px).
- **Node title color**: `module-node.svelte` uses `resolveNodeColor(data, moduleDef)` from `module.ts` — `data.color` (per-instance) > `moduleDef.hints.color` (per-definition) > kind-based fallback. Supports both palette indices (1-7) and hex codes (`#rrggbb`). Changeable via inspector (single node, hex supported) or context menu Color submenu (multi-node batch, palette only). `KIND_COLOR_DEFAULTS` in `module.ts` is the shared kind→palette-index mapping used by both inspector and context menu "Apply to ports".
- **Node background color**: `bg_color` extension > `moduleDef.hints.bg_color`, no fallback (null = default gray). When set, both body and title bar are opaque by default; a UI package can override the body via a `nodeStyles` entry keyed on def_name (e.g. Note gets an 85% translucent body). Disabled (`bg-muted`) and unknown-definition (`bg-destructive`) styling takes precedence.
- **Node foreground color**: `fg_color` extension > `moduleDef.hints.fg_color`, no fallback (null = theme default). Applied as `style:color` on the node body in `node-base.svelte`, so body text (incl. markdown) inherits it; the title bar text stays governed by `color`, and port labels keep their own colors. Same disabled/unknown-definition suppression as `bg_color`. Note ships `fg_color="#44403b"` so its sticky body stays readable in dark theme.
- **Port colors**: `port_colors` extension (`Record<string, number | string>`) overrides per-port handle/label colors and edge stroke colors from that port. Inspector provides "Apply to ports" (copies title color to all non-err ports) and "Clear". Edge creation in all 5 code paths (`patchToFlow`, `handleBeforeConnect`, `AddConnectionCommand.execute`, `DeleteCommand.undo`, `PasteCommand.execute`) passes `sourcePortColors` to `connectionSpecToEdge`. `refreshEdgeColorsForNode()` on `EditorState` updates existing edges when `port_colors` changes (including undo/redo).
- **Per-instance extensions**: `EXTENSION_KEYS` registry in `inspector-state.svelte.ts` lists extension keys (currently `["color", "port_colors", "bg_color", "fg_color"]`); adding a new extension = add key + add UI in `inspector.svelte` + add rendering where the node is drawn (`module-node.svelte` or `node-base.svelte`). Inspector sync uses `_lastSyncedData` reference comparison to detect `updateNodeData` changes on the same selected node (enables undo/redo sync without ad-hoc patches).
- **Extension persistence**: `ModuleSpec.extensions` (`#[serde(flatten)]`) stores extension values; `updateModuleSpec` with `null` removes the key (Rust `spec.rs` `update()` calls `shift_remove` on null). Frontend sends `undefined` to `updateNodeData` and `null` to `updateModuleSpec` when clearing.
...
- **`hotkeys.ts`**: Central module — `DEFAULT_HOTKEYS` definitions, `matchHotkey()` matcher, `resolveHotkeys()` / `resolveQuickAddModules()` resolvers, `formatHotkey()` display formatter.
...
- **Settings storage**: `shortcut_keys` in `CoreSettings` (Rust `HashMap<String, String>`). Quick Add module assignments use `quick_add.N.module` keys in the same map.
- **Key format**: `mod+s` (mod = Cmd on macOS, Ctrl on Win/Linux), `shift+a`, `f11`. Sequences: `"a 1"` (space-separated chords, 500ms timeout).
- **Quick Add**: `mod+1`–`mod+5` place modules at mouse position. Both key binding and module type are user-customizable.
...
- **`setCoreSettings(Partial<CoreSettings>)`** (`module.ts`): Merges into `_coreSettings` cache + updates `coreSettingsStore` optimistically before the backend IPC call; backend `autostart::apply()` runs outside Mutex lock to prevent deadlock.
...
- **ID remapping**: Backend assigns new IDs when modules are re-created (redo of Add, undo of Delete). `CommandHistory.redo()`/`undo()` propagates ID changes to remaining stack commands via `Command.remapId()`. All 12 command classes implement `remapId`.
- **Config coalescing**: `pushCoalescing()` merges rapid config changes (same node+key within 500ms) into one undo entry.
- **Not undoable**: Viewport pan/zoom, snap toggle, patch start/stop, selection changes, backend-initiated config updates (`moduleEvent.configUpdated`).

Command classes (13): `AddModuleCommand`, `DeleteCommand`, `CutCommand` (extends Delete), `AddConnectionCommand`, `MoveNodesCommand` (also for Align/Distribute), `ResizeNodeCommand`, `PasteCommand`, `UpdateConfigCommand`, `UpdateTitleCommand`, `UpdateExtensionCommand`, `BatchUpdateExtensionCommand` (multi-node color/port_colors via context menu), `ToggleDisabledCommand`, `ToggleShowErrCommand`.
...
$effect(() => {
  const { message, seq } = moduleEvent.error;
  if (!seq) return;
...
When the filesystem under `~/.modular_agent/patches/` changes, the backend emits `ma:patch_list_changed` so the sidebar refreshes automatically.

| Command                   | Emit condition                                                                               |
| ------------------------- | -------------------------------------------------------------------------------------------- |
| `new_patch_with_name_cmd` | Always (saves empty `PatchSpec::default()` to disk immediately)                              |
| `save_as_patch_cmd`       | Always (uses `add_patch_with_name` to load spec into core engine + register name→ID mapping) |
| `save_patch_cmd`          | Only when file is new (`!patch_path_exists`)                                                 |
| `delete_patch_cmd`        | Always                                                                                       |
| `delete_folder_cmd`       | Always (the deleted folder's parent)                                                         |
| `import_patch_cmd`        | Always                                                                                       |
| `rename_patch_cmd`        | Always (also emits `ma:patch_renamed` if patch is open)                                      |
| `rename_folder_cmd`       | Always (also emits `ma:patch_renamed` per affected open patch)                               |

- **Rename as primitive**: `rename_patch(name, new_name)` and `rename_folder(path, new_path)` are the core methods; `move_patch` and `move_folder` are thin wrappers that compute `new_name` from `target_dir` + basename.
- **Delete**: `delete_folder(path)` only removes an **empty** folder (`remove_dir`) — a right-click lands on the wrong row too easily for a recursive delete to be safe, so a non-empty folder fails with "Delete its contents first". `delete_patch` refuses while the patch is running ("Stop it first", same policy as rename/move) and drops its `auto_start_patches` entry via `remove_auto_start_patches`. Neither prunes emptied ancestor directories (rename/move do) — deleting removes exactly the row that was clicked.
- **Ancestor notification**: When `save_patch_cmd` or `new_patch_with_name_cmd` creates a new subdirectory (via `create_dir_all`), an additional event is emitted for the grandparent directory.
- **Helper**: `parent_patch_path(name)` extracts the parent directory from a patch name (e.g., `"Cat/MyPatch"` → `"Cat"`).
- **Frontend**: `PatchTreeStore` (`patch-tree-store.svelte.ts`) is a singleton ViewModel with `$effect.root()` listener (same pattern as `SharedModuleEvents`). Refreshes the directory at `event.payload.path` if it is in the loaded `entries` record; otherwise walks up to the nearest loaded ancestor and refreshes that instead (a new folder appeared on disk — e.g. an MCP `save_patch` into a fresh subdirectory — and must show up in an already-loaded parent). Uses per-path request counter to discard stale responses from concurrent events. `nav-patches.svelte` is a pure View that reads `patchTreeStore.entries` — no direct Tauri event handling. Uses keyed `{#each entries as entry (entry)}` to preserve folder open/close state across refreshes.
- **Patch file list UI**: VS Code-style full-row interaction — indentation via `depth` prop and `padding-left` (formula: `8 + depth * 16` px) instead of nested margins, so the entire row (left edge to right edge) is clickable, draggable, and right-clickable. Hover uses `hover:bg-sidebar-accent`. Tooltip uses native `title` attribute with patch-relative path (not OS path).

## MCP Server (External Agent Editing)

`modular-agent-core`'s built-in MCP server (feature `mcp-server`) lets external AI agents (Claude Code, Codex) read and edit patches. Disabled by default.
...
- **Tools** (17, defined in core `src/mcp_server.rs`): `list_module_definitions`, `get_module_definition`, `list_patches`, `create_patch`, `get_patch_spec`, `add_module`, `update_module_spec`, `set_module_configs`, `remove_module`, `add_connection`, `remove_connection`, `save_patch`, `start_patch`, `stop_patch`, `write_external_input`, `get_module_errors`, `get_external_outputs`. Failures a module can fix itself return `is_error` tool results with corrective hints (valid handle lists, `PatchNameExists` guidance), not protocol errors. The two polling tools read a shared 200-entry ring of `Error`/`ExternalOutput` events; `latest_seq` is the seq of the last _returned_ record (echoes `since_seq` on an empty page) — cursor semantics, not a global counter — plus a `dropped` count for broadcast-lag losses.
- **Origin convention**: every core mutation entry point stamps `EventEnvelope.origin` via `ModularAgent::with_origin` — `"desktop"` (tauri-plugin handle), `"mcp"` (MCP server handles), `None` (module runtime internal). `shared.svelte.ts` defines `isExternalOrigin(origin)`: anything but `"desktop"` is external (null counts as external). Self-echo filtering is purely origin-based — there are no frontend suppression wrappers around the plugin APIs.
- **Event relay** (`observer.rs`): subscribes to `EventEnvelope`s and forwards `origin` in every Tauri payload. Besides the four module events, it relays `PatchStructureChanged` → `ma:patch_structure_changed { origin, patch_id }`, `PatchRemoved` → `ma:patch_removed { origin, patch_id, name }`, `PatchRenamed` → `ma:patch_renamed` plus `ma:patch_list_changed` for old/new parent folders, and `PatchAdded` (named) / `PatchSaved` → `ma:patch_list_changed { path }` via `parent_patch_path` — so MCP-side create/save/rename refresh the sidebar. `ma:patch_removed` for an open tab calls `closeTabAndNavigate(patch_id)`; editor-host cleanup then unloads the already-removed patch idempotently. Like `ma:patch_renamed`, it is deliberately **not** origin-filtered — a sidebar delete arrives as our own `"desktop"` echo, and this is the only path that closes the tab. The only other desktop-origin removal is `close_patch` (fired _after_ the tab is gone), so it no-ops.
- **Name index in core**: core owns the name→ID mapping (`patch_names`, `find_patch_id_by_name`); the desktop keeps no map of its own. Name collisions on create/rename fail with `Error::PatchNameExists`, surfaced to the UI (and hinted to MCP callers). `ModularAgentApp::open_patch` returns the live instance when the name is already loaded in core (e.g. MCP-created) instead of loading a duplicate from the file; `close_patch` calls core `remove_patch` (idempotent). `remove_module` also removes spec-only modules (unknown definitions, no runtime instance) and still emits `PatchStructureChanged`. `get_patch_spec` keeps spec-only modules (it overlays live specs onto the stored entries rather than rebuilding from instances, so a save cannot drop them), and `update_module_spec` / `set_module_configs` fall back to patching the stored spec entry when there is no instance.
- **Diff merge (external edit → canvas)**: core emits `PatchStructureChanged { patch_id }` from all structure mutations (incl. `update_module_spec` when the patch has non-`configs` keys) → `shared.svelte.ts` bumps `sharedPatchEvents.structureChanged[patch_id]` (per-patch seq, external origins only) → each `EditorState` `$effect` compares against `lastStructureSeq` (initialized from `baseStructureSeq`, captured by the host _before_ the flow fetch — a reopened tab doesn't replay old events, a change landing mid-fetch still merges) → 300ms debounce (`scheduleExternalMerge`, re-arms while a drag/resize/command is in flight) → `applyExternalChanges()` fetches `getPatchInfo` + `getPatchSpec`, then re-checks after the IPC awaits: `mergeGen` generation token (newest run wins), tab still open, and `history.mutationSeq` unchanged (a local edit that landed mid-fetch discards the stale snapshot and re-arms) → `reconcileFlow()` (`merge.ts`) merges the fetched flow identity-preservingly — nodes keyed by module id (unchanged nodes keep their object, array order, and `selected`), edges keyed by connection tuple — and returns `removedModuleIds`/`removedConnKeys`. On change: apply nodes/edges, `history.purgeInvalidated(...)`, `savedIndex = -1` (backend diverged from last save). Undo history and selection survive. Any merge failure falls back to `reloadFromBackend()` — full `patchToFlow` rebuild + `history.clear()`.
- **Undo purge policy**: `CommandHistory.purgeInvalidated` runs `Command.pruneExternalRemovals(removedModuleIds, removedConnKeys)` over both stacks; returning `false` drops the command, and commands may instead trim themselves partially — `MoveNodesCommand`/`BatchUpdateExtension`/`ToggleDisabled`/`ToggleShowErr`-style commands filter their per-node deltas (dropped only when empty), `DeleteCommand`/`PasteCommand` filter saved edges by `removedConnKeys` but drop entirely if a saved node (or an endpoint of a saved edge) was removed, single-node commands (config/title/extension/resize) drop when their node is gone. Commands without the hook are untouched.
- **Cargo note**: `src-tauri/Cargo.toml` enables the `mcp-server` feature on `modular-agent-core`. core and the Tauri plugin are in-tree workspace dependencies, and out-of-tree modules reach core by path from their `custom_modules/` clone, so nothing in the workspace patches crates.io. ma-config codegen keeps the feature on regen.

## Module Plugins

Module functionality is provided by external crates linked in `lib.rs`:
...
cloned from their own repositories into `custom_modules/` at the workspace root and linked
from there by ma-config.

## Custom Build (ma-config)

`tools/ma-config/` at the workspace root is a TUI wizard for selecting which module packages a build links. It serves both apps: `ma-config desktop` and `ma-config cli`.

- Codegen generates `src-tauri/src/modules.rs` (do not edit manually) and updates `src-tauri/Cargo.toml`; `lib.rs` has `mod modules;` to link them.
- core and the Tauri plugin are in-tree and always linked, so they are not selectable. In-tree modules (std / llm) are emitted as `{ workspace = true }`; a feature override spells the path out instead, because cargo ignores a member's `default-features = false` when the workspace entry does not set it.
- Out-of-tree modules are emitted as `path = "../../../custom_modules/<name>"` — clone them there by hand first (`custom_modules/README.md` lists the repositories), since the wizard only offers clones that exist; ma-config fails pointing at that README when a selected clone has gone missing, and with a fix-up message when a clone still depends on `modular-agent-core` from crates.io (two copies of core, no visible modules). Custom modules outside the registry keep an inline `path` of their own in `[dependencies]`.
- Configuration is saved to `apps/desktop/ma-config.toml` (gitignored) with paths relative to the workspace root. The catalog is split: `tools/ma-config/registry.yaml` lists the in-tree modules (std / llm) only, and each out-of-tree crate carries its own entry — description, features, conflicts, default selection — in an `ma-registry.yaml` at the root of its clone, which the wizard picks up by scanning `custom_modules/`.

## HTML Sanitization

All `{@html}` rendering must go through `src/lib/sanitize.ts`:

- **`sanitizeHtml(html)`**: DOMPurify with restricted ALLOWED_TAGS/ALLOWED_ATTR. Use for HTML content from modules.
