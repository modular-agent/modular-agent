...
- **Node background color**: `bg_color` extension > `agentDef.hints.bg_color`, no fallback (null = default gray). When set, the body is rendered 85% translucent and the title bar opaque; disabled (`bg-muted`) and unknown-definition (`bg-destructive`) styling takes precedence.
