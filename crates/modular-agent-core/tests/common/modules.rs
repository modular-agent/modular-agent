use modular_agent_core::{
    AsModule, Error, ModularAgent, ModuleConfigSpec, ModuleConfigSpecs, ModuleConfigs,
...
/// cancellation. Used to verify that stop_module does not block behind a
...
pub struct StuckSleepModule {
    data: ModuleData,
}

#[async_trait]
impl AsModule for StuckSleepModule {
    ...
    async fn process(&mut self, ctx: ModuleContext, _port: String, _value: Value) -> Result<()> {
        self.output(ctx.clone(), PORT_OUT, Value::string("started"))
            .await?;
...
pub struct CancelWaitModule {
    data: ModuleData,
}

#[async_trait]
impl AsModule for CancelWaitModule {
    fn new(ma: ModularAgent, id: String, spec: ModuleSpec) -> Result<Self> {
        Ok(Self {
            data: ModuleData::new(ma, id, spec),
        })
    ...
    async fn process(&mut self, ctx: ModuleContext, _port: String, _value: Value) -> Result<()> {
        ...
        let Some(token) = ctx.cancel_token().cloned() else {
            return Err(Error::Other("no cancel token in context".into()));
        };
        tokio::select! {
            _ = token.cancelled() => {
                self.output(ctx, PORT_OUT, Value::string("aborted")).await
            }
...
/// its timeout instead of hanging on a misbehaving module.
#[modular_agent(
    title = "Pending Stop",
    category = CATEGORY,
    inputs = [PORT_IN],
...
pub struct PendingStopModule {
    data: ModuleData,
}

#[async_trait]
impl AsModule for PendingStopModule {
    fn new(ma: ModularAgent, id: String, spec: ModuleSpec) -> Result<Self> {
        Ok(Self {
            data: ModuleData::new(ma, id, spec),
        })
...
/// Models modules like ZipToObject that generate configs/ports at
...
/// Match modules, whose live spec - not their definition - knows which config
...
pub struct NumberedConfigModule {
    data: ModuleData,
}
...
/// from `n`.
fn update_numbered_spec(spec: &mut ModuleSpec) -> Result<usize> {
    let n = spec
        .configs
        .as_ref()
    ...
    configs.set(CONFIG_N.to_string(), Value::integer(n as i64));
    config_specs.insert(CONFIG_N.to_string(), n_spec);

    for i in 0..n {
        let name = format!("c{}", i);
        // `ModuleDefinition::reconcile_spec` parks every config the definition
...
#[async_trait]
impl AsModule for NumberedConfigModule {
    fn new(ma: ModularAgent, id: String, mut spec: ModuleSpec) -> Result<Self> {
...
impl AsModule for CounterModule {
    ...
    async fn start(&mut self) -> Result<()> {
        self.count = 0;
        // self.emit_display(DISPLAY_COUNT, ModuleData::new_integer(0))?;
        Ok(())
    }
    ...
    async fn process(&mut self, ctx: ModuleContext, port: String, _value: Value) -> Result<()> {
        ...
        self.output(ctx, PORT_COUNT, Value::integer(self.count))
            .await?;
        // self.emit_display(DISPLAY_COUNT, Value::integer(self.count))?;
        Ok(())
    }
}
