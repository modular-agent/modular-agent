...
async fn test_rename_onto_used_name_fails() {
    ...
    let id_a = ma.new_patch_with_name("a".into()).unwrap();
    ...
    assert_eq!(ma.find_patch_id_by_name("b"), Some(id_b));

    ma.quit();
}

#[tokio::test]
async fn test_remove_patch_frees_name() {
    let ma = ModularAgent::init().unwrap();
    ma.ready().await.unwrap();

    let id1 = ma.new_patch_with_name("reuse".into()).unwrap();
    ma.remove_patch(&id1).await.unwrap();
    assert_eq!(ma.find_patch_id_by_name("reuse"), None);

    let id2 = ma.new_patch_with_name("reuse".into()).unwrap();
    assert_ne!(id1, id2);
    assert_eq!(ma.find_patch_id_by_name("reuse"), Some(id2));
...
async fn test_rename_to_own_name_succeeds() {
    ...
    let id = ma.new_patch_with_name("same".into()).unwrap();
    ma.rename_patch(&id, "same".into()).await.unwrap();
    assert_eq!(ma.find_patch_id_by_name("same"), Some(id));
