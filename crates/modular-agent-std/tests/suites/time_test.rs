...
use ma::{ConnectionSpec, ModularAgent, ModuleConfigs, ModuleContext, Value};

const DELAY_DEF: &str = "modular_agent_std::time::DelayModule";
const INTERVAL_DEF: &str = "modular_agent_std::time::IntervalTimerModule";
const ON_START_DEF: &str = "modular_agent_std::time::OnStartModule";
const SCHEDULE_DEF: &str = "modular_agent_std::time::ScheduleTimerModule";
const THROTTLE_DEF: &str = "modular_agent_std::time::ThrottleTimeModule";
const PROBE_DEF: &str = "modular_agent_core::test_utils::TestProbeModule";

// Timer emissions run in real time on the shared module runtime, so these tests use
...
/// Wire one module of `def_name` (configs adjusted by `configure`) to a probe on
...
async fn setup_module_with_probe(
    ...
    (patch_id, module_id, probe)
}

#[tokio::test]
async fn test_delay_reemits_in_order() {
    let ma = test_utils::setup_modular_agent().await;
    let (_patch_id, delay_id, probe) = setup_module_with_probe(&ma, DELAY_DEF, "value", |cfg| {
        cfg.set("delay".into(), Value::integer(50));
    })
    .await;

    let ctx = ModuleContext::new();
    let module = ma.get_module(&delay_id).unwrap();
    {
        let mut guard = module.lock().await;
        guard
            .process(ctx.clone(), "value".into(), Value::integer(1))
            .await
            .unwrap();
        guard
            .process(ctx.clone(), "value".into(), Value::integer(2))
            .await
            .unwrap();
    }
...
async fn test_interval_timer_ticks_and_stops() {
    let ma = test_utils::setup_modular_agent().await;
    let (patch_id, _module_id, probe) = setup_module_with_probe(&ma, INTERVAL_DEF, "unit", |cfg| {
        cfg.set("interval".into(), Value::string("100ms"));
    })
    .await;
...
async fn test_on_start_emits_unit_once() {
    let ma = test_utils::setup_modular_agent().await;
    let (_patch_id, _module_id, probe) =
        setup_module_with_probe(&ma, ON_START_DEF, "unit", |cfg| {
            cfg.set("delay".into(), Value::integer(50));
        })
        .await;
...
async fn test_schedule_timer_fires_on_cron() {
    ...
    let (_patch_id, _module_id, probe) =
        setup_module_with_probe(&ma, SCHEDULE_DEF, "time", |cfg| {
            cfg.set("schedule".into(), Value::string("* * * * * * *"));
        })
        .await;
...
/// Injects `values` into the throttle in one burst while holding the module lock.
async fn burst(ma: &ModularAgent, module_id: &str, ctx: &ModuleContext, values: &[i64]) {
    let module = ma.get_module(module_id).unwrap();
    let mut guard = module.lock().await;
    for v in values {
        guard
            .process(ctx.clone(), "value".into(), Value::integer(*v))
            .await
            .unwrap();
    }
}
...
async fn test_throttle_capacity0_leading_edge_only() {
    ...
    let (_patch_id, throttle_id, probe) =
        setup_module_with_probe(&ma, THROTTLE_DEF, "value", |cfg| {
            cfg.set("interval".into(), Value::string("200ms"));
            cfg.set("capacity".into(), Value::integer(0));
        })
        .await;
...
async fn test_throttle_capacity1_keeps_newest() {
    ...
    let (_patch_id, throttle_id, probe) =
        setup_module_with_probe(&ma, THROTTLE_DEF, "value", |cfg| {
            cfg.set("interval".into(), Value::string("200ms"));
            cfg.set("capacity".into(), Value::integer(1));
        })
        .await;
